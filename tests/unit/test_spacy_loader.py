...
def test_get_nlp_downloads_and_loads_on_oserror():
    ...
    with patch('spacy.load', side_effect=[OSError, mock_nlp]) as mock_spacy_load, \
        ...
        assert nlp_instance == mock_nlp
        assert SpacyModelLoader._nlp == mock_nlp
        mock_logger_info.assert_any_call(f"spaCy model '{SpacyModelLoader._model_name}' not found. Attempting to download...")
        mock_logger_info.assert_any_call(f"Successfully downloaded and loaded spaCy model '{SpacyModelLoader._model_name}'.")

def test_get_nlp_exits_on_download_failure():
    # Reset the _nlp instance before each test to ensure isolation
    SpacyModelLoader._nlp = None
    # Arrange
    with patch('spacy.load', side_effect=OSError) as mock_spacy_load, \
